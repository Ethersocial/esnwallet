...
const ClientBinaryManager = require('./ethersocial-client-binaries').Manager;
...
exports.mocha = (_module, options) => {
    ...
    _module.exports[options.name || path.basename(_module.filename)] = {
        * before() {
            ...
            const appFileName = (options.app === 'wallet') ? 'EtherSocial Wallet' : 'Mist';
            const platformArch = `${process.platform}-${process.arch}`;

            let appPath;
            const ipcProviderPath = path.join(this.geth.dataDir, 'gesc.ipc');
